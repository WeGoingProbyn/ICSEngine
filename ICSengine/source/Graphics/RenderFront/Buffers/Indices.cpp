...
Indices::Node::Node(char* ptr, Indices::Hierachy& hierachy)
	:
	m_Ptr(ptr),
	m_Hierachy(hierachy)
{
...
Indices::Node Indices::IndexBuffer::End()
{
	...
	return Node{ &m_ByteBlob[0] + m_ByteBlob.Size() - m_Hierachy.GetIndexHierachy()[m_TotalBufferedNodes].GetSize(), m_Hierachy};
}

Indices::Node Indices::IndexBuffer::Begin()
{
	ICS_ASSERT_MSG(m_ByteBlob.Size() > 0, "Vertices: Buffer is nullptr, cannot return on Begin");
	return Node{ &m_ByteBlob[0], m_Hierachy};
}
...
Indices::Node Indices::IndexBuffer::operator[](unsigned int index)
{
	...
	return Node{ &m_ByteBlob[0] + m_Hierachy.GetIndexHierachy()[index].GetOffset(), m_Hierachy};
}
